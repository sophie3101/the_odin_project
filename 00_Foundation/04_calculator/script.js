...
const num_and_op_btns = document.querySelectorAll(
  "button.calc-op, button.calc-num"
);
...
const end_is_op = (input) =>
  "+-/x".indexOf(input.charAt(input.length - 1)) !== -1;
...
const loadColor = (theme_num) => {
  ...
  num_and_op_btns.forEach(
    (button) =>
      (button.style.cssText = `background-color:${color_obj[theme_num].key_num_bg};color:${color_obj[theme_num].text_key}; box-shadow:0 5px ${color_obj[theme_num].key_num_shadow}`)
  );

  redo_buttons.forEach(
    (button) =>
      (button.style.cssText = `background-color:${color_obj[theme_num].key_reset_bg};box-shadow:0 5px ${color_obj[theme_num].key_reset_shadow}`)
  );
...
const calculatorListener = (e) => {
  ...
  switch (class_name) {
    ...
    case "calc-del": // if all_input end in operation, delete operation
      ...
      user_input.all_input = user_input.all_input.substring(
        0,
        user_input.all_input.length - 1
      );
      break;
    default:
      console.log("out");
